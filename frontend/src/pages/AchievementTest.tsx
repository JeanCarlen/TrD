...
const AchTest: React.FC = () => {
  ...
  const GetUserAchieve = async () => {
    ...
    const response = await fetch(
      `http://localhost:8080/api/userachievments/users/${content.user}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
  ...
  return (
    <div>
      ...
      <button onClick={GetAchieve}>NO ACHIEVE?</button>
      <button onClick={CreateAchieve}>ADD USER ACHIEVE</button>
    </div>
  );
};
