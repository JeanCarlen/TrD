...
export class FriendsService {
  ...
  public async findFriendsList(id: number) {
	return await this.friendsRepository.find({where: [{ requester: id, status: 1}, { requested: id, status: 1 }]})
  }

  public async findPendingFriends(id: number) {
	return await this.friendsRepository.find({where: { requester: id, status: 0}})
  }

  public async findPendingRequests(id: number) {
	return await this.friendsRepository.find({where: { requested: id, status: 0}})
  }
  ...
  public async update(id: number, updateFriendDto: UpdateFriendDto) {
	const friends: Friends = await this.friendsRepository.findOne({where: { id: id}});
